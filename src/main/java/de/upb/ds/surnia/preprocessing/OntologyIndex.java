...
public class OntologyIndex {
  ...
  private org.slf4j.Logger log = LoggerFactory.getLogger(OntologyIndex.class);
  ...
  /**
   * Search ontology index for matches.
   * @param object String to find matches for.
  ...
  /**
   * Close reader and directory of the index.
   */
  public void close() {
    try {
      ireader.close();
  ...
  private void addDocumentToIndex(Resource r, String p, String o) throws IOException {
    Document doc = new Document();
    doc.add(new StringField(FIELD_NAME_SUBJECT, r.getURI(), Store.YES));
    doc.add(new StringField(FIELD_NAME_PREDICATE, p, Store.YES));
    doc.add(new TextField(FIELD_NAME_OBJECT, o, Store.YES));
